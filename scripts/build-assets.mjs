...
const renderEvents = (events) => {
  ...
  // A date-neutral, stable no-JS fallback. Runtime selects the next event or latest past record.
  const event = sortedEvents.at(-1);
...
const bundledStyles = (await transform(authoredStyles, { loader: "css", minifyWhitespace: true, charset: "utf8" })).code;
const scriptSource = await readFile(join(repositoryRoot, "src/script.js"), "utf8");
const script = (await transform(scriptSource, { loader: "js", minifyWhitespace: true, charset: "utf8" })).code;
...
const content = JSON.parse(await readFile(join(repositoryRoot, "content/site.json"), "utf8"));
const arcaSvg = await readFile(join(repositoryRoot, "templates/arca-network.svg"), "utf8");
let renderedHtml = html;
for (const [id, block] of Object.entries(renderContent(content, arcaSvg))) {
  renderedHtml = replaceSingle(renderedHtml, new RegExp(`<!-- GENERATED:${id}:start -->[\\s\\S]*?<!-- GENERATED:${id}:end -->`),
    `<!-- GENERATED:${id}:start -->\n${block}\n<!-- GENERATED:${id}:end -->`, `generated ${id}`);
}
const women = content.projects.find(project => project.id === "women");
const archive = content.projects.find(project => project.id === "archive");
renderedHtml = renderedHtml.replace(/<div\b[^>]*data-archive-stack\b[^>]*>/g, tag => {
  const project = tag.includes("archive-stack--women") ? women : archive;
  const kind = project === women ? "photographs" : "archival records";
  return tag.replace(/aria-label="[^"]*"/, `aria-label="${escapeHtml(`${project.images.length} ${kind} from ${project === archive ? 'the ' : ''}${project.title}`)}"`);
})
  .replace(/(<span data-archive-register-kind>)[^<]*/, (_, tag) => tag + escapeHtml(archive.images[0].kind))
  .replace(/(<span data-archive-register-year>)[^<]*/, (_, tag) => tag + escapeHtml(archive.images[0].year))
  .replace(/<div class="archive-register"[^>]*>/, `<div class="archive-register" data-archive-register aria-hidden="true" style="--archive-register-count: ${archive.images.length}">`)
  .replace(/<div class="archive-register__rail">[\s\S]*?<\/div>/, `<div class="archive-register__rail"><i class="archive-register__marker"></i>${archive.images.map((_, index) => `<span>${String(index + 1).padStart(2, "0")}</span>`).join("")}</div>`);
...
if (checkOnly) {
  ...
  if (await readFile(scriptPath, "utf8") !== script) failures.push("script.js is not built from src/script.js.");
